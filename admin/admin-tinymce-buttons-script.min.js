...
g=[];jQuery(".sb"+b+" .tg_searchbox .tg_container form").find("input, select, textarea").each(function(){g.push({name:this.name,value:this.value,obj:this})});var h=jQuery("#travelSearchUseJavaScript").attr("checked"),d="",e=tgsbFromAir(g),d=d+(e.length?e+",":""),e=tgsbToAir(g),d=d+(e.length?e+",":""),e=tgsbHotelCity(g),d=d+(e.length?e+",":""),e=tgsbDepartureDate(g),d=d+(e.length?e+",":""),e=tgsbReturnDate(g),d=d+(e.length?e+",":""),e=tgsbAdults(g),d=d+(e.length?e+",":""),e=tgsbKids(g),d=d+(e.length?
e+",":""),e=tgsbSeniors(g),d=d+(e.length?e+",":""),e=tgsbRooms(g),d=d+(e.length?e+",":""),e=tgsbRTOW(g),d=d+(e.length?e+",":""),d=d+tgsbCruises(g),d=d+(h?'"usejavascript":"on",':"")+("300x250"==b?"":'"size":"'+b+'",'),d=d+(null==c?'"selectedTab":"flights",':"flights"==c[1]?"":'"selectedTab":"'+c[1]+'",');alignment=setAlignment();d+="alignnone"==alignment?"":'"alignment":"'+alignment+'"';d.match(/\,$/)&&(d=d.replace(/\,$/,""));b="[tg_searchboxes"+(d.length?" options='{"+d+"}']":"]");b=a._do_tg_searchboxes(b);
tinyMCE.execCommand("mceInsertContent",0,b);"undefined"!=typeof tgsb_selBoxParam&&tgsb_selBoxParam&&(tgsb_selBoxParam=!1);tb_remove()}return!1});return!1}jQuery(document).ready(function(a){tinymce.create("tinymce.plugins.TGSearchboxes",tgsb_popupInitOpt);tinymce.PluginManager.add("TGSearchboxes",tinymce.plugins.TGSearchboxes)});